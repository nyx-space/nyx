...
where
    ...
    <DefaultAllocator as Allocator<f64, <D::StateType as State>::VecLength>>::Buffer: Send,
{
    pub fn run<F: EventEvaluator<D::StateType>>(
        self,
        template: D::StateType,
        end_condition: Either<Epoch, (Duration, &F)>,
        num_runs: usize,
    ) {
        let cosm = Cosm::de438();
        ...
        init_states
            ...
            .for_each_with(arc_prop, |setup, state| {
                let final_state = setup.with(template).until(end_condition);
...
fn test_mc_setup() {
    ...
    let prop_time: Epoch = dt + 1 * TimeUnit::Day;
    my_mc.run(state, Either::Left(prop_time), 100);
}
