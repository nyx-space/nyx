...
pub fn dv_pointing_error<R: Rng>(
    ...
    let cur_angle = (orbit.velocity().dot(&dv_hat) / orbit.vmag()).acos();
...
pub fn dv_execution_error<R: Rng>(
    ...
    let dv = dv_pointing_error(orbit, dv, pointing_3s, rng)?;
...
fn test_dv_mag_fixed() {
    ...
    for _ in 0..=1000 {
        ...
        let dv_w_err = dv_pointing_error(&orbit, dv, 0.1, &mut thread_rng()).unwrap();
        assert!(
