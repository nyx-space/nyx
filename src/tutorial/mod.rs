...
// You can get the last state from the output of the for call
let last_state_0 = prop.for_duration(prop_time).unwrap();
...
let backprop_state = prop.for_duration(-prop_time).unwrap();
...
let last_state = prop.for_duration(SECONDS_PER_DAY).unwrap();
...
thread::spawn(move || {
    ...
    prop.for_duration(2.0 * SECONDS_PER_DAY).unwrap();
});
