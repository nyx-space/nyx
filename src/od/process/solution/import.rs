    ...
    <DefaultAllocator as Allocator<<Spacecraft as State>::Size, <Spacecraft as State>::Size>>::Buffer<f64>: Copy,
{
    ...
    pub fn from_parquet<P: AsRef<Path>>(path: P, devices: BTreeMap<String, Trk>) -> Result<Self, InputOutputError> {
        ...
        ensure!(has_epoch, MissingDataSnafu { which: "Epoch (UTC)" });
        ...
        let mut filter_smoother_ratios: Vec<Option<OVector<f64, <Spacecraft as State>::Size>>> = Vec::new();
        ...
        for record_batch in reader {
            ...
            let x_col = get_col("X (km)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting X",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let y_col = get_col("Y (km)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting Y",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let z_col = get_col("Z (km)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting Z",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let vx_col = get_col("VX (km/s)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting VX",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let vy_col = get_col("VY (km/s)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting VY",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let vz_col = get_col("VZ (km/s)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting VZ",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();

            let cr_col = get_col("cr")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting Cr (unitless)",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let cd_col = get_col("cd")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting Cd (unitless)",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();

            let dry_mass_col = get_col("dry_mass (kg)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting dry_mass (kg)",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
            let prop_mass_col = get_col("prop_mass (kg)")?
                .as_any()
                .downcast_ref::<Float64Array>()
                .ok_or_else(|| InputOutputError::ArrowError {
                    action: "downcasting prop_mass (kg)",
                    source: arrow::error::ArrowError::CastError("".to_string()),
                })?
                .clone();
                ...
                for j in i..state_size {
                    ...
                    cov_cols.push(
                        get_col(col_name)?
                            ...
                            .downcast_ref::<Float64Array>()
                            .ok_or_else(|| InputOutputError::ArrowError {
                                action: "downcasting covariance column",
                                source: arrow::error::ArrowError::CastError("".to_string()),
                            })?
                            .clone(),
                    );
            ...
            let rejected_col = get_col("Residual Rejected")
                .ok()
                .and_then(|arr| arr.as_any().downcast_ref::<BooleanArray>().cloned());
            let tracker_col = get_col("Tracker")
                .ok()
                .and_then(|arr| arr.as_any().downcast_ref::<StringArray>().cloned());
            let ratio_col = get_col("Residual ratio")
                .ok()
                .and_then(|arr| arr.as_any().downcast_ref::<Float64Array>().cloned());
            ...
            for i in 0..state_size {
                for f in &measurement_types_found {
                    ...
                    let base_name = format!("Gain {}*{f:?} ({}*{})", state_items[i], cov_units[i], f.unit());
                    let col_name = schema
                        .fields()
                        .iter()
                        .find(|f| f.name().starts_with(&base_name))
            ...
            for i in 0..num_rows {
                let epoch = Epoch::from_gregorian_str(epoch_col.value(i)).map_err(|e| InputOutputError::Inconsistency {
                ...
                let nominal_state = Spacecraft::builder()
                    ...
                    .mass(Mass::from_dry_and_prop_masses(dry_mass_col.value(i), prop_mass_col.value(i)))
                    .srp(SRPData {
                ...
                let is_rejected_opt = rejected_col
                    .as_ref()
                    .and_then(|col| if col.is_valid(i) { Some(col.value(i)) } else { None });
                let tracker_opt = tracker_col.as_ref().and_then(|col| {
                    if col.is_valid(i) {
                ...
                let ratio_opt = ratio_col
                    .as_ref()
                    .and_then(|col| if col.is_valid(i) { Some(col.value(i)) } else { None });

                let current_residual: Option<Residual<MsrSize>> = if let (Some(is_rejected), Some(tracker), Some(ratio)) =
                    (is_rejected_opt, tracker_opt.clone(), ratio_opt)
                {
                    ...
                    for (msr_type, type_cols) in &residual_data_cols {
                        if msr_idx >= MsrSize::DIM {
                            break;
                        } // Should not happen if MsrSize matches data
                        ...
                        let postfit_val =
                            type_cols
                                .get("Postfit residual")
                                .and_then(|col| if col.is_valid(i) { Some(col.value(i)) } else { None });
                        let noise_val = type_cols.get("Measurement noise").and_then(|col| {
                            if col.is_valid(i) {
                                Some(col.value(i))
                            } else {
                                None
                        ...
                        let real_val =
                            type_cols
                                ...
                                .and_then(|col| if col.is_valid(i) { Some(col.value(i)) } else { None });
                        let comp_val = type_cols.get("Computed observation").and_then(|col| {
                            if col.is_valid(i) {
                                Some(col.value(i))
                            } else {
                                None
                ...
                let current_gain: Option<OMatrix<f64, <Spacecraft as State>::Size, MsrSize>> =
                    if gain_available && !gain_cols.is_empty() {
