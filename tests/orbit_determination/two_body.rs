...
//     let mut init_est_state = initial_state;
//     init_est_state.enable_stm();
//     let prop_est = Propagator::new::<RK4Fixed>(&orbital_dyn, opts).with(init_est_state);
...
//     let prop_est = Propagator::new::<RK4Fixed>(&tb_estimator, PropOpts::with_fixed_step(step_size))
//         .with(init_est_state);
//     let covar_radius = 1.0e-3;
//     let covar_velocity = 1.0e-6;
//     let init_covar = Matrix6::from_diagonal(&Vector6::new(
...
//     let prop_est = Propagator::new::<RK4Fixed>(&estimator, opts).with(init_est_state);
...
//     init_est_state.enable_stm();
//     let tb_estimator = OrbitalDynamics::two_body();
//     let prop_est = Propagator::new::<RK4Fixed>(&tb_estimator, opts).with(init_est_state);
