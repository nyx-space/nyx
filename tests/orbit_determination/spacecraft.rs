...
use self::nyx::dimensions::{Matrix2, Matrix6, SMatrix, SVector, Vector2, Vector6, U9};
...
#[allow(clippy::identity_op)]
#[test]
fn od_val_sc_mb_cr_est() {
    /*
     * This tests that we can correctly estimate the Cr parameter of a spacecraft.
     *
     **/
    if pretty_env_logger::try_init().is_err() {
        println!("could not init env_logger");
    ...
    while let Ok(rx_sc_state) = truth_rx.try_recv() {
        for station in all_stations.iter() {
            let meas = station.measure(&rx_sc_state).unwrap();
            if meas.visible() {
                measurements.push(meas);
                break;
            }
    ...
    let covar_cr = 1e-3;
    let covar_cd = 1e-3;
    let covar_fuel_mass = 1e-6;
    let init_covar = SMatrix::<f64, 9, 9>::from_diagonal(&SVector::<f64, 9>::from_row_slice(&[
        covar_radius,
        covar_radius,
        covar_radius,
        covar_velocity,
    ...
    for (no, est) in odp.estimates.iter().enumerate() {
        ...
        if !printed {
            // Format the estimate
            wtr.serialize(estimate_fmtr.fmt(est.orbit))
                .expect("could not write to stdout");
            printed = true;
        }
    }
