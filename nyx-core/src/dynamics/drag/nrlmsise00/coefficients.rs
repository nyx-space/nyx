...
//!
//! Source: NRLMSISE-00 official distribution, U.S. Naval Research Laboratory.
//! Reference: Picone, J.M. et al. (2002), "NRLMSISE-00 empirical model of the
//!   atmosphere: Statistical comparisons and scientific issues",
//!   J. Geophys. Res., 107(A12), 1468, doi:10.1029/2002JA009430
//! Mathematical framework: Hedin, A.E. (1991), J. Geophys. Res., 96(A2), 1159-1172.
//!
//! Values extracted from the NRL official distribution as published data.
...
//!
//! Generated by: tools/extract_nrlmsise00_coefficients.py
//! Source file: nrlmsise-00_data.c (C transcription by Dominik Brodowski,
//!   based on official NRL Fortran distribution DIST12.TXT)

/// Exospheric temperature variation coefficients (150 terms).
///
...
/// Density variation coefficients for each species (9 species x 150 terms).
///
/// Each row is a 150-term coefficient array used in `geographic_variation()`
/// to compute the fractional perturbation of species density.
...
/// Temperature gradient (S-parameter) variation coefficients (150 terms).
///
/// Used in `geographic_variation()` to compute the temperature gradient
/// that controls the shape of the Bates-Walker exponential profile.
...
/// Composition and turbopause correction parameters (2 x 25 terms).
///
/// Scale heights, altitude thresholds, and mixing ratios used in
/// `composition_correction()` and `composition_correction_dual()` for
/// each species' departure from ideal diffusive equilibrium.
...
/// Temperature boundary and reference values (50 terms).
///
/// Base values for exospheric temperature, lower boundary temperature,
/// temperature at spline nodes, gradient parameters, and reference altitudes.
...
/// Density boundary parameters per species (8 species x 10 terms).
///
/// Each row contains reference density, mixing ratio, reference altitude,
/// correction amplitude, correction altitude, scale height, and
/// chemistry correction parameters for one species.
/// Species order: He, O, N2, O2, Ar, H, N, anomalous O.
...
/// Temperature node correction coefficients (4 nodes x 100 terms).
///
/// Each row is a 100-term coefficient array used in `geographic_variation_lower()`
/// to compute the fractional perturbation of a lower thermosphere temperature node.
/// Node order: TN1(2) at 110 km, TN1(3) at 100 km, TN1(4) at 90 km, TN1(5) at 72.5 km.
...
/// Middle atmosphere variation coefficients (10 x 100 terms).
///
/// Each row is a 100-term coefficient array used in `geographic_variation_lower()`
/// for mesosphere/stratosphere temperature nodes and gradients.
...
/// Semiannual multiplier coefficients (100 terms).
...
/// Middle atmosphere average values (10 terms).
...
mod tests {
    ...
    fn sam_first_half_ones_second_half_zeros() {
        // SAM_COEFFICIENTS is 50 ones followed by 50 zeros
        for i in 0..50 {
            assert!(
