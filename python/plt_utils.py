...
def plot_with_error(fig,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                color=
                f"rgb({int(color[0]*0.85)}, {int(color[1]*0.85)}, {int(color[2]*0.85)})"
            ),
            line=dict(width=0),
            showlegend=False,
        ))
...
def plot_line(fig, df, x, y_name, color_name, ytitle, title, copyright):
    ...
    plot_raw_line(fig, x, df[y_name], y_name, color_name, ytitle, title,
                  copyright)


def plot_raw_line(fig,
                  x,
                  y,
                  y_name,
                  color_name,
...
def build_sphere(size, color, num_points=500, opacity=1.0):
    ...
    trace = go.Surface(
        ...
        colorscale=[
            [
                0,
                f"rgba({int(color[0])}, {int(color[1])}, {int(color[2])}, {opacity})"
            ],
            [
                1,
                f"rgba({int(color[0])}, {int(color[1])}, {int(color[2])}, {opacity})"
